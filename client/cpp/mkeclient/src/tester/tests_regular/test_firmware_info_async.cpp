...
SCENARIO("firmware_info_async") {
  ...
  GIVEN("no connection") {
    THEN("getting info") { 
      gResults[gRid++].checkGetFirmwareInfo(client, MKE_SHOULD_DO_FATAL);
    }
  }
  ...
  GIVEN("with TCP connection") {
    ...
    WHEN("when state is " << test_state) {
      ...
      THEN("getting info") {
        ...
        gRid++;
        gResults[gRid].checkGetFirmwareInfo(client, MKE_SHOULD_WORK);
        mke::api::MkEReply_FirmwareInfo info = gResults[gRid].firmware_info;
